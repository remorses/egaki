...
cli
  ...
  .action(async (entry, options) => {
    ...
    console.log()
    console.log(`  ${pc.dim('SDK available in browser console as')} ${pc.yellow('window.egakiSDK')}`)
    console.log(`    ${pc.dim('.seekTo(frame)')}       ${pc.dim('jump to a frame')}`)
    console.log(`    ${pc.dim('.screenshot()')}        ${pc.dim('capture frame as data URL')}`)
    console.log(`    ${pc.dim('.filmstrip({...})')}     ${pc.dim('grid of frames across scenes')}`)
    console.log(`    ${pc.dim('.export({...})')}        ${pc.dim('render MP4 via WebCodecs')}`)
    console.log(`    ${pc.dim('.getInfo()')}            ${pc.dim('composition metadata')}`)
    console.log()
    console.log(`  ${pc.dim('Docs:')} ${pc.cyan('https://raw.githubusercontent.com/remorses/egaki/main/cli/skills/egaki/SKILL.md')}`)
    console.log()
    if (options.open) {
